...
This is a copy bundled into the kernel container so it can import
...
def validate_dna(dna: DNA) -> list[str]:
    """Return a list of warnings. Empty list means DNA is valid."""
    warnings = []
    if not dna.guardians:
        warnings.append("No guardians found in DNA")
    if not dna.skills:
        warnings.append("No skills found in DNA")
    if not dna.identity:
        warnings.append("Missing Identity section")
    if not dna.first_objective:
        warnings.append("Missing First Objective section")
    expected_guardians = {"Queen", "Antking", "Wiseoldant", "Nurse"}
    found = {g.name for g in dna.guardians}
    missing = expected_guardians - found
    if missing:
